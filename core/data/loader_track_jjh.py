...
class DriverDataJJH(torch.utils.data.Dataset):
    def __init__(self, driver_path, feature_data_list=None, point_plane_size=296):
        super().__init__()
        ...
        if feature_data_list is None:
            ...
            # _lmdb_engine.close()
            raise ValueError("Please provide a feature_data_list for now.")
        else:
            self.f_images = []
            self.f_shapes = []
            self.f_planes = []
            for data in self.feature_data_list:
                f_image = torchvision.transforms.functional.resize(data['image'].cpu(), (518, 518), antialias=True)
                f_transform = data['transform_matrix'].float().cpu()
                f_plane = build_points_planes(point_plane_size, f_transform)
                f_shape = data["shapecode"].float().cpu()
                self.f_images.append(f_image)
                self.f_shapes.append(f_shape)
                self.f_planes.append(f_plane)
    ...
    def _load_one_record(self, frame_key):
        ...
        t_image = self._lmdb_engine[frame_key].float() / 255.0
        t_points = self.flame_model(
            shape_params=self.f_shapes[5][None], pose_params=this_record['posecode'][None],
            expression_params=this_record['expcode'][None], eye_pose_params=this_record['eyecode'][None],
        )[0].float()
        one_data = {
            'f_images': deepcopy(self.f_images), 'f_planes': deepcopy(self.f_planes), 
