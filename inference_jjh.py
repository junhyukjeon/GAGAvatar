...
def inference(image_dir, driver_path, resume_path, force_retrack=False, device='cuda'):
    ...
    # build input data
    feature_name = os.path.basename(image_dir).split('.')[0]
    feature_data_list = []
    ...
    if feature_data_list is None:
        print(f'Finish inference, no face in input: {image_dir}.')
        return
    ...
    if os.path.isdir(driver_path):
        ...
        driver_dataset = DriverDataJJH(driver_path, feature_data_list, meta_cfg.DATASET.POINT_PLANE_SIZE)
        ...
        if driver_data is None:
            ...
            return
        driver_dataset = DriverDataJJH({driver_name: driver_data}, feature_data_list, meta_cfg.DATASET.POINT_PLANE_SIZE)
        driver_dataloader = torch.utils.data.DataLoader(driver_dataset, batch_size=1, num_workers=2, shuffle=False)
...
if __name__ == '__main__':
    ...
    inference(args.image_dir, args.driver_path, args.resume_path, args.force_retrack)